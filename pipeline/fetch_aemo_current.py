...
def main() -> int:
    ...
    # Trailing-edge 404 tolerance: NEMWeb CURRENT's ~48 h dispatch retention
    # means the oldest zip in our window may roll off mid-run, producing a
    # legitimate 404 that's not a pipeline failure. Treat low-rate 404s as
    # warnings; anything else (or 404 rate > 1 %) stays fatal so a genuinely
    # broken URL pattern still fails the cron.
    fatal = [(u, e) for u, e in failed
             if "404 Client Error" not in e]
    edge_404 = [(u, e) for u, e in failed
                if "404 Client Error" in e]
    edge_threshold = max(1, len(zips) // 100)  # 1 % of attempted zips, floor 1

"""
Load monthly rooftop parquet files into raw.rooftop_pv_30min via Postgres COPY FROM STDIN.
...
Boundary semantics
------------------
Same (start, end] window as load_to_postgres.py — see that file for the
rationale on interval-ENDING timestamps and month classification.

Prerequisites
-------------
1.  cp .env.example .env  ; fill in DB_PWD
2.  psql -d <DB_NAME> -f db/01_raw_schema.sql
3.  python pipeline/fetch_rooftop.py     # produce parquet files first

Usage
-----
    python pipeline/load_rooftop.py                    # all parquet files
    python pipeline/load_rooftop.py --start 2024-01 --end 2024-12
    python pipeline/load_rooftop.py --month 2024-12
    python pipeline/load_rooftop.py --force            # reload existing months
"""
...
from _common import ROOFTOP_COLS

# -----------------------------------------------------------------------------
# Config
# -----------------------------------------------------------------------------
PROJECT_ROOT = Path(__file__).resolve().parent.parent
PARQUET_DIR = PROJECT_ROOT / "data" / "parquet" / "rooftop_30min"

# Load .env at import time so any os.getenv() below sees the values.
load_dotenv(PROJECT_ROOT / ".env")

PARQUET_NAME_RE = re.compile(r"rooftop_30min_(\d{4})-(\d{2})\.parquet$")

TABLE = "rooftop_pv_30min"  # schema name comes from $DB_SCHEMA env var

# Postgres column order = lowercased ROOFTOP_COLS in same order. Must match
# db/01_raw_schema.sql column order.
COLUMNS_LOWER = [c.lower() for c in ROOFTOP_COLS]
...
def list_parquet_files(start: str | None, end: str | None, month: str | None):
    ...
    out = []
    for p in sorted(PARQUET_DIR.glob("rooftop_30min_*.parquet")):
...
def month_bounds(year: int, month: int) -> tuple[str, str]:
    ...
    Same (start, end] semantics as load_to_postgres.py — see that file
    for the rationale.
    """
...
def month_already_loaded(cur, schema: str, year: int, month: int) -> int:
    ...
    cur.execute(
        ...
        f"WHERE interval_datetime > %s AND interval_datetime <= %s",
        (start_excl, end_incl),
    )

...
- **Data source for deployment.** This analysis uses AEMO MMSDM Historical archives (monthly publication, ~2-week lag) and Open-Meteo ERA5 (5-day lag) — appropriate for retrospective batch work. Real-time deployment of the Finding 8 classifier is out of scope here; it would require swapping the fetchers to NEMWeb CURRENT (same `DISPATCHREGIONSUM` / `DISPATCHPRICE` / `ROOFTOP_PV_ACTUAL` tables at ~5–10-minute latency) and substituting BoM observations + 24-hour forecast for ERA5. The leak-free feature design is compatible with this swap — only the source endpoints change.
